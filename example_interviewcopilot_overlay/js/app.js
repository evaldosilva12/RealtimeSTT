...
function handleEvent(event) {
    ...
    if (event.type === "connection.status") {
        ...
        if (event.profiles || event.active_profile) {
            renderProfiles(event.profiles || [], event.active_profile || null, event.openai_setup);
        }
    ...
    if (event.type === "profiles.current") {
        renderProfiles(event.profiles, event.active_profile, event.openai_setup);
        return;
    }
